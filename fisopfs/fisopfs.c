...
int fisopfs_getattr(const char *path, struct stat *stbuf) {
...
printf("[debug] fisopfs_readdir - path: %s\n", path);
    ...
    return fs_readdir_entries(path, buffer, filler);
}

int fisopfs_read(const char *path, char *buffer, size_t size, off_t offset, struct fuse_file_info *fi){
